...
Namespaces, class names, and property names are limited to 256 characters; classes to 4,096; properties to 256 per class or instance; instances to 1,024; string values to 64 KiB; array elements to 256; and aggregate result text to 8 million characters. Provider errors remain contextual `WmiInspectionException` failures, not empty results. WMI read access does not prove method-execution authorization, complete visibility, or maliciousness.
