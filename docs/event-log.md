...
Channel snapshots are limited to 4,096 entries and 32,768 UTF-16 characters per name. Queries require an explicit maximum from 1 through 4,096. Rendered XML is capped at 1 MiB; oversized output fails rather than being truncated. EventData extraction is limited to 512 fields, 256-character names, and 16 KiB values.
...
The laboratory is local-only and does not modify logs, security descriptors, tokens, privileges, memory, or processes. x64, x86, and ARM64 builds are compile-time validated; runtime behavior requires validation on the matching Windows host.
