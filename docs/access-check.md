...
ACL inspection tells you what rules exist. AccessCheck evaluates a specific token against those rules. AccessCheck does not provide a complete explanation of every authorization decision: DACL ACE ordering, token SIDs, privileges, requested access, and other Windows rules matter. This implementation evaluates only the current process against files/directories; registry/service evaluation, ACL modification, authorization explanation, and actual impersonation are out of scope.
